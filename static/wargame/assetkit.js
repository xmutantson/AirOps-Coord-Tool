...
(function () {
  ...
  const manifest = {
    ...
    props: {
      ...
      plane: { png: `${ROOT}/props/plane.png`, json: `${ROOT}/props/plane.json` } // optional atlas
    }
  };
  ...
  function ensurePlaceholders(scene) {
    ...
    const makePlayerDir = (key, tri) => {
      if (tex.exists(key)) return;
      const g = scene.add.graphics(); const w = 48, h = 64;
    ...
    if (!tex.exists('ph:player')) {
      ...
      g.generateTexture('ph:player', w, h); g.destroy();
    }
    ...
    // --- NEW: example art with plane BODY and WINGS as separate textures ----
    const makePlaneBody = (key, w, h) => {
      if (tex.exists(key)) return;
      const g = scene.add.graphics();
      // fuselage
      g.fillStyle(0xd9e3e8, 1).fillRoundedRect(0, 0, w, h, 10);
      // center stripe
      g.fillStyle(0xa7b5be, 1).fillRect(w*0.40, 4, w*0.20, h-8);
      // cockpit
      g.fillStyle(0x5c7c8a, 1).fillRoundedRect(6, h*0.28, 22, h*0.44, 6);
      // tail
      g.fillStyle(0xa7b5be, 1).fillRoundedRect(w-18, h*0.20, 12, h*0.60, 4);
    ...
    const makePlaneWings = (key, w, h) => {
      if (tex.exists(key)) return;
      const g = scene.add.graphics();
      // broad wings slab with slight notch
      const wingH = Math.max(18, Math.floor(h*0.38));
      g.fillStyle(0xbfcad0, 1).fillRoundedRect(0, h*0.31, w, wingH, 6);
      // darker leading edge
      g.fillStyle(0x9fb0ba, 1).fillRoundedRect(4, h*0.31+4, w-8, wingH-8, 5);
      // flaps hint
      g.lineStyle(2, 0x81929c, 0.8);
      const flapY = h*0.31 + wingH*0.5;
      for (let i=1;i<5;i++) g.lineBetween(i*(w/5), flapY-8, i*(w/5), flapY+8);
      g.generateTexture(key, w, h);
      g.destroy();
    };
    // One set (we'll flip for facing left)
    makePlaneBody('ph:plane-body', 140, 64);
    makePlaneWings('ph:plane-wings', 160, 64);
  }

  window.AssetKit = { manifest, queueOptional, ensurePlaceholders };
})();
